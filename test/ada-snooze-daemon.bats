#!/usr/bin/env bats
# Tests for lib/ada-snooze-daemon.py — the project's one real trust boundary.
...
setup() {
  load test_helper
  setup_common
  DAEMON="$REPO_ROOT/lib/ada-snooze-daemon.py"
...
@test "dismiss exits without re-arming" {
  export ADA_SNOOZE_LOG="$BATS_TEST_TMPDIR/snooze.log"
  read -r port token < <(spawn_daemon 5) || { echo "daemon never came up"; false; }
  http_code "http://127.0.0.1:$port/$token/dismiss" >/dev/null
  wait_for_file "$ADA_SNOOZE_LOG" 100
  # the daemon logs its terminal decision; dismiss => "exit without snooze"
  run bash -c "grep -q 'exit without snooze' '$ADA_SNOOZE_LOG'"
  assert_success
}
...
@test "an out-of-range snooze is rejected (no re-arm)" {
  ...
  wait_for_file "$ADA_SNOOZE_LOG" 100
  run bash -c "for _ in \$(seq 1 60); do grep -q 'exit without snooze' '$ADA_SNOOZE_LOG' && exit 0; sleep 0.1; done; exit 1"
  assert_success
  run bash -c "grep -q 'relaunch after sleep' '$ADA_SNOOZE_LOG'"
