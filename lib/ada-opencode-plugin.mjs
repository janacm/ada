...
// opencode resolves import.meta.url to the REALPATH, so under Homebrew this is
// <prefix>/Cellar/ada/<version>/libexec/lib even though the shim imports via
// the version-stable opt path. `brew upgrade` deletes that keg, which would
// leave a long-running opencode spawning a missing file. Map Cellar back to opt
// when the opt path exists (the same rule as __ada_stable_dir in the installer).
const stableDir = (dir) => {
  const m = dir.match(/^(.*)\/Cellar\/([^/]+)\/[^/]+\/(.+)$/)
  if (!m) return dir
  const opt = join(m[1], "opt", m[2], m[3])
  return existsSync(opt) ? opt : dir
}
const LIB_DIR = stableDir(dirname(fileURLToPath(import.meta.url)))
...
const oneLine = (value) => {
  ...
  // Clip by code point, not UTF-16 unit, so an emoji is never cut in half.
...
// keyboard to stop it, so neither the error alert nor the finish alert is worth
// a window. Checked BEFORE the ADA_OPENCODE_EVENTS gate: suppressing your own
// interruption is turn lifecycle, not a feature of the error category, and
// dropping "error" from the event list must not resurrect the spurious alert.
const isSelfInflicted = (error) => error?.name === "MessageAbortedError"

const errorLabel = (error) => {
  if (!error) return null
  const { name, data } = error

  // Aborts are handled by isSelfInflicted() before this is ever called; there
  // is deliberately no second abort rule here to drift out of sync with it.
...
// broken opencode.
const fire = (rawLabel, elapsedSeconds, repoDir, sessionID) => {
  if (process.platform !== "darwin") return
  // Clip AFTER composition, not before: callers prepend "⚠️ Error: " and
  // "🔐 Needs permission: " to text that may already be MAX_LABEL long, and
  // ada-claude-hook.sh makes the same guarantee about its own composed label.
  ...
  try {
    ...
    const child = spawn(NOTIFY, [label, elapsedSeconds == null ? "" : String(elapsedSeconds), "0"], {
      ...
      env: {
        ...
        // Lets the alert's "Mute this session" button silence this session;
        // the launcher drops alerts for a muted key (lib/ada-mute.sh). A
        // failure with no session id gets no key, and so no button.
        ADA_SESSION_KEY: sessionID ? `opencode-${sessionID}` : "",
        ADA_SESSION_KIND: "session",
      },
    })
...
export const AdaAlert = async ({ directory }) => {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      // A message sent while a turn is still open (a queued follow-up) must not
      // restart the clock or drop a stashed error: keep the turn's earliest
      // start until session.idle closes it.
      const open = turns.get(sessionID)
      if (open?.start) return
      turns.set(sessionID, {
        start: Date.now(),
        prompt: open?.prompt || promptFromParts(output?.parts),
        error: open?.error ?? null,
        silenced: open?.silenced ?? false,
      })
    ...
    event: async ({ event }) => {
      ...
      switch (type) {
        ...
        // A long-lived `opencode serve` can create sessions all day; without
        // this the per-session maps only ever grow.
        case "session.deleted": {
          const gone = props.info?.id ?? props.sessionID
          if (!gone) return
          turns.delete(gone)
          children.delete(gone)
          directories.delete(gone)
          return
        }

        case "session.error": {
          const sessionID = props.sessionID
          debug(`session.error sid=${sessionID ?? ""} name=${props.error?.name ?? ""}`)

          if (isSelfInflicted(props.error)) {
            const aborted = sessionID ? turns.get(sessionID) : null
            if (aborted) aborted.silenced = true
            return
          }

          if (!EVENTS.has("error")) return
          const label = errorLabel(props.error)
          // An error payload we cannot describe (the sdk types make `error`
          // itself optional) must leave the turn ALONE rather than silence it —
          // otherwise an empty error event swallows the finish alert for a turn
          // that really did run for ten minutes.
          if (!label) return
          ...
          const turn = turns.get(sessionID) ?? { start: null, prompt: "", silenced: false }
          turn.error = label
        ...
        case "session.idle": {
          ...
          if (turn.error) {
            fire(`⚠️ Error: ${turn.error}`, elapsed, dirFor(sessionID), sessionID)
            return
          }
          if (!EVENTS.has("finish") || elapsed == null) return
          if (elapsed < THRESHOLD) return
          fire(turn.prompt || "opencode", elapsed, dirFor(sessionID), sessionID)
          return
        }
        ...
        case "permission.updated": {
          ...
          // 1.18.30 sends `patterns: string[]`; the 1.18.20 sdk types declare
          // `pattern?: string | string[]`. Take whichever is present, and its
          // first element when it is a list, so an array never reaches the
          // label as "a,b" via String().
          const rawPattern = props.patterns ?? props.pattern
          const firstPattern = Array.isArray(rawPattern) ? rawPattern[0] : rawPattern
          const detail = props.metadata?.command ?? firstPattern ?? props.title
          ...
          fire(`🔐 Needs permission: ${label}`, null, dirFor(props.sessionID), props.sessionID)
          return
        }
